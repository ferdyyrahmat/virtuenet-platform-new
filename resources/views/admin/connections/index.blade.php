...
<div class="container-fluid platform-page">
    ...
    <div class="row g-4">
        @foreach([
            ...
            <div class="col-xl-4"><div class="card platform-card h-100"><div class="card-body p-4">
                <div class="connection-header"><span class="connection-icon"><i class="mdi {{ $provider==='litellm'?'mdi-creation':($provider==='github'?'mdi-github':'mdi-message-processing-outline') }}"></i></span><div><h6>{{ $meta[0] }}</h6><span class="connection-status is-{{ $connection?->health_status ?? 'unknown' }}">{{ str($connection?->health_status ?? 'not configured')->replace('_',' ')->title() }}</span></div></div><p class="text-muted small my-3">{{ $meta[1] }}</p>
                <form method="POST" action="{{ route('admin.connections.update',$provider) }}">@csrf @method('PUT')<input type="hidden" name="label" value="{{ $meta[0] }}"><div class="form-check form-switch mb-3"><input type="hidden" name="enabled" value="0"><input class="form-check-input" type="checkbox" id="enabled-{{ $provider }}" name="enabled" value="1" @checked($connection?->enabled)><label class="form-check-label" for="enabled-{{ $provider }}">Enable connection</label></div>
                    ...
                    <label class="form-label mt-3">{{ $meta[3] }}</label><input type="password" name="secret" class="form-control" autocomplete="new-password" placeholder="{{ $connection && data_get($connection->credentials,$provider==='litellm'?'master_key':($provider==='lark'?'app_id':'token')) ? 'Saved — leave blank to keep' : 'Enter credential' }}">
