    ...
    <div class="row g-4">
        @foreach([
            ...
            <div class="col-xl-3 col-md-6"><div class="card platform-card h-100"><div class="card-body p-4">
                <div class="connection-header"><span class="connection-icon"><i class="mdi {{ $provider==='litellm'?'mdi-creation':($provider==='github'?'mdi-github':($provider==='github_lark_sync'?'mdi-source-branch-sync':'mdi-message-processing-outline')) }}"></i></span><div><h6>{{ $meta[0] }}</h6><span class="connection-status is-{{ $connection?->health_status ?? 'unknown' }}">{{ str($connection?->health_status ?? 'not configured')->replace('_',' ')->title() }}</span></div></div><p class="text-muted small my-3">{{ $meta[1] }}</p>
